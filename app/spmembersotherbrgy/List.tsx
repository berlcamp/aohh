...
function List({ barangays}: { barangays: string[]}) {
  ...
  const fetchMembers = async () => {
    ...
    try {
      await axios.get(`${apiUrl}/aorv/spmembersotherbrgy`, { params })
        .then(response => {
      ...
      await axios.put(`${apiUrl}/aorv/updatespid`, params)
        .then((response: any) => {
          const d = data.map((item) => {
            if (item.id === id) {
              return { ...item, sp_id: response.data.sp_id, sp_fullname: response.data.sp_fullname, sp_address: response.data.sp_address, error_message: response.data.error_message }
            }
            return item
          })
          setData(d)
  ...
  return (
    <div>
      <div className="text-center text-2xl text-gray-300">Other Barangay SP</div>
      {
      ...
      { loading && <Loading/> }
      {
        (!loading && data.length > 0) &&
          <div className='w-full flex flex-col border bg-gray-200 px-4 py-2'>
            <div className='text-center mt-4 mb-10'>List of voters in {filterBarangay} with SP from other barangay</div>
            <table>
              <thead>
                  ...
                  data.map((item: VotersType, index) => (
                    <tr key={index} className='border-b border-gray-300'>
                      ...
                      <td className='text-[10px] px-1 py-2'>
                        ...
                        <div className='text-green-700'>{item.sp_address}</div>
